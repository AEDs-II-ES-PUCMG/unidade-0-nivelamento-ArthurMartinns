...
public class Pedido {
	...
	public double valorFinal() {
		...
		for (int i = 0; i < quantProdutos; i++) {
			valorPedido += produtos[i].getProduto().valorDeVenda();
		}
	...
	public void mesclarPedido(Pedido outroPedido) {
		if(outroPedido.produtos.length > MAX_PRODUTOS) {
			throw new IllegalStateException("Quantidade acima do maximo permitido!");
		}


		for (int i = 0; i < produtos.length; i++) {
			if(this.equals(outroPedido)) {
				
				produtos[i].getPreco() += outroPedido.produtos[i].getPreco();
			}else {

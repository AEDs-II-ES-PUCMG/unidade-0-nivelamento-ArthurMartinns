...
public abstract class Produto {
	...
	static Produto criarDoTexto(String linha) {
		...
		if (Integer.parseInt(subString[0]) == 1) {
			novoProduto = new ProdutoNaoPerecivel(subString[1], Double.parseDouble(subString[2]),
					Double.parseDouble(subString[3]));
		} else {
			LocalDate dataValidade = LocalDate.parse(subString[4], formatter);
			novoProduto = new ProdutoPerecivel(subString[1], Double.parseDouble(subString[2]),
					Double.parseDouble(subString[3]), dataValidade);
		}
		return novoProduto;
	}
